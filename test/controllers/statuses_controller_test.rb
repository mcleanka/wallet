...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference("Status.count") do
      post statuses_url, params: { status: { description: @status.description, name: @status.name } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { description: @status.description, name: @status.name } }
